...
public class Chunk
{
    ...
    private readonly Color[,] _lightColors; // Cached RGB light values
    ...
    public Chunk(int chunkX, int chunkY)
    {
        ...
        _lightColors = new Color[Size, Size];
        
        // Initialize light colors to white by default (will be updated by lighting system)
    ...
    /// <summary>
    /// Get cached light level at local coordinates (average of RGB)
    /// </summary>
    public byte GetLightLocal(int localX, int localY)
    {
        if (localX < 0 || localX >= Size || localY < 0 || localY >= Size)
            return 0;
        Color c = _lightColors[localX, localY];
        return (byte)((c.R + c.G + c.B) / 3);
    }
    
    /// <summary>
    /// Get cached light color at local coordinates
    /// </summary>
    public Color GetLightColorLocal(int localX, int localY)
    {
        if (localX < 0 || localX >= Size || localY < 0 || localY >= Size)
            return Color.Black;
        return _lightColors[localX, localY];
    }
    
    /// <summary>
    /// Set light level at local coordinates (grayscale)
    /// </summary>
    public void SetLightLocal(int localX, int localY, byte light)
    {
        if (localX < 0 || localX >= Size || localY < 0 || localY >= Size)
            return;
        _lightColors[localX, localY] = new Color(light, light, light);
    }
    
    /// <summary>
    /// Set light color at local coordinates
    /// </summary>
    public void SetLightColorLocal(int localX, int localY, Color color)
    {
        if (localX < 0 || localX >= Size || localY < 0 || localY >= Size)
            return;
        _lightColors[localX, localY] = color;
    }
    ...
    public void Draw(SpriteBatch spriteBatch, Camera2D camera)
    {
        ...
        for (int y = startY; y < endY; y++)
        {
            for (int x = startX; x < endX; x++)
            {
                ...
                // Draw full bright (lighting handled by overlay)
                spriteBatch.Draw(TextureManager.TileAtlas, destRect, sourceRect, Color.White);
            }
        }
    }
    ...
    {
         if (!camera.IsVisible(Bounds)) return;
        ...
        for (int y = startY; y < endY; y++)
        {
            for (int x = startX; x < endX; x++)
            {
                var tile = _tiles[x, y];
                // Check if emissive
                if (tile == TileType.Torch || tile == TileType.Lava || tile == TileType.Furnace)
                {
                    int worldX = (TileStartX + x) * GameConfig.TileSize;
                    int worldY = (TileStartY + y) * GameConfig.TileSize;
                    var sourceRect = TextureManager.GetTileRect(tile);
